 ...
 * from the `Config` type in the published `@washy-washy/core`). Unpinned to
 * the package's `latest` npm tag rather than the running version, matching
 * how most `$schema` URLs work: close enough, without needing a build-time
 * read of this package's own `package.json`.
 */
